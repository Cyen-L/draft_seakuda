...
def main():
    ...
    try:
        ...
        ELD.write('POWER 150') # Set ELD to 150W for discharge mode
        PWA1.write(encode('INPUT:MODE AC')) # Set PWA1 to AC mode for discharge measurements
        PWA1.write(encode('INPUT:VOLTAGE:RANGE 600V')) # Set PWA1 voltage range to 600V
        PWA2.write(encode('INPUT:MODE AC')) # Set PWA2 to AC mode for discharge measurements
        PWA2.write(encode('INPUT:VOLTAGE:RANGE 600V')) # Set PWA2 voltage range to 600V
        ...
        # Retrive battery life (SOC) and state code from the unpacked data
        data = struct.unpack('<31h', temp)
        battery_life = data[-7]
        state_code = data[11] * 0.1 # According to Fransico script
        ...
        while True:
            ...
            # Switch to discharge high load mode if battery life is above 100%
            if battery_life >= 100 and mode == 'charge':
                PSU.write(encode('OUTP OFF'))
                ELD.write('POWER 250')
                mode = 'discharge-hl'
                cycle_count += 1

            # Switch to discharge reduce load mode if battery life is below 15%
            if battery_life <= 15 and mode == 'discharge-hl':
                ELD.write('POWER 180')
                mode = 'discharge-rl'

            # If battery life is below 15% and status code is in 2.1
            if battery_life <= 15 and mode == 'discharge-rl' and state_code == 2.1:
                mode = 'waiting for charge'
                discharge_rl_start_time = datetime.datetime.now()

            # Get the waiting time after status code in 2.1 is triggered
            discharge_rl_time = datetime.datetime.now() - discharge_rl_start_time
            time_check = discharge_rl_time > datetime.timedelta(minutes=1)

            # If we are waiting for charge and the time check is passed, switch to charge mode
            if mode == 'waiting for charge' and time_check:
                PSU.write(encode('OUTP ON'))
                ELD.write('POWER 150')
                mode = 'charge'
